...
export default class BattleScene extends SceneBase {
  ...
  async initVariantData(): Promise<void> {
    clearVariantData();
    const otherVariantData = await this.cachedFetch(
    ...
    const expVariantData = await this.cachedFetch(
  ...
  async initStarterColors(): Promise<void> {
    ...
    const sc = await this.cachedFetch("./starter-colors.json").then((res) =>
      res.json(),
    );
  ...
  addPokemonIcon(
    ...
    const icon = this.add.sprite(
      0,
      0,
      pokemon.getIconAtlasKey(ignoreOverride, useIllusion),
    );
    ...
    if (pokemon.isFusion(useIllusion)) {
      const fusionIcon = this.add.sprite(
        0,
        0,
        pokemon.getFusionIconAtlasKey(ignoreOverride, useIllusion),
      );
  ...
  isNewBiome(currentBattle = this.currentBattle) {
    ...
    const isEndlessFifthWave =
      this.gameMode.hasShortBiomes && currentBattle.waveIndex % 5 === 0;
    const isWaveIndexMultipleOfFiftyMinusOne =
      currentBattle.waveIndex % 50 === 49;
    const isNewBiome =
    ...
    if (
      ...
      if (
        ...
        newBattleType =
          ...
          battleType ??
          (this.gameMode.isWaveTrainer(newWaveIndex, this.arena)
            ? BattleType.TRAINER
            : BattleType.WILD);
      }
      ...
      if (newBattleType === BattleType.TRAINER) {
        const trainerType =
          Overrides.RANDOM_TRAINER_OVERRIDE?.trainerType ??
          this.arena.randomTrainerType(newWaveIndex);
  ...
  setFieldScale(scale: number, instant = false): Promise<void> {
    return new Promise((resolve) => {
      ...
      this.tweens.add({
        ...
        duration: !instant
          ? fixedInt(Math.abs(this.field.scale - scale) * 200)
          : 0,
        ease: "Sine.easeInOut",
        onComplete: () => resolve(),
      });
  ...
  getSpeciesFormIndex(
    ...
    if (
      ...
      if (
        species.speciesId === Species.SILVALLY ||
        species.speciesId === Species.ARCEUS
      ) {
  ...
  generateRandomBiome(waveIndex: number): Biome {
    ...
    const biomes = getEnumValues(Biome).filter(
  ...
  canTransferHeldItemModifier(
    ...
    return (
      !removeOld ||
      !source ||
      this.hasModifier(itemModifier, !source.isPlayer())
    );
  }
  ...
  getMysteryEncounter(
    ...
    for (const seenEncounterData of this.mysteryEncounterSaveData
      .encounteredEvents) {
