...
export function getPokemonSpeciesForm(
  species: Species,
  formIndex: number,
): PokemonSpeciesForm {
...
export function getFusedSpeciesName(
  speciesAName: string,
  speciesBName: string,
): string {
  ...
  const [speciesAPrefixMatch, speciesBPrefixMatch] = [
    speciesAName,
    speciesBName,
  ].map((n) => /^(?:[^ ]+) /.exec(n));
  const [speciesAPrefix, speciesBPrefix] = [
    speciesAPrefixMatch,
    speciesBPrefixMatch,
  ].map((m) => (m ? m[0] : ""));
  ...
  const [speciesASuffixMatch, speciesBSuffixMatch] = [
    speciesAName,
    speciesBName,
  ].map((n) => / (?:[^ ]+)$/.exec(n));
  const [speciesASuffix, speciesBSuffix] = [
    speciesASuffixMatch,
    speciesBSuffixMatch,
  ].map((m) => (m ? m[0] : ""));
  ...
  if (splitNameB.length === 1) {
    if (fragBMatch) {
      ...
      fragB =
        (/[\-']/.test(prevCharB) ? prevCharB : "") + fragBMatch[2] || prevCharB;
...
export abstract class PokemonSpeciesForm {
  ...
  getRootSpeciesId(forStarter = false): Species {
    ...
    while (
      pokemonPrevolutions.hasOwnProperty(ret) &&
      (!forStarter || !speciesStarterCosts.hasOwnProperty(ret))
    ) {
  ...
  getPassiveAbility(formIndex?: number): Abilities {
    ...
    while (
      ...
      if (pokemonPrevolutions.hasOwnProperty(starterSpeciesId)) {
        ...
        console.log(
          "No passive ability found for %s, using run away",
          this.speciesId,
        );
  ...
  getLevelMoves(): LevelMoves {
    if (
      ...
      pokemonSpeciesFormLevelMoves[this.speciesId].hasOwnProperty(
        this.formIndex,
      )
    ) {
      return pokemonSpeciesFormLevelMoves[this.speciesId][this.formIndex].slice(
        0,
      );
    }
  ...
  isObtainable(): boolean {
    return (
      this.generation <= 9 || pokemonPrevolutions.hasOwnProperty(this.speciesId)
    );
  }

  isCatchable(): boolean {
    return (
      this.isObtainable() && uncatchableSpecies.indexOf(this.speciesId) === -1
    );
  }
  ...
  isTrainerForbidden(): boolean {
    return [Species.ETERNAL_FLOETTE, Species.BLOODMOON_URSALUNA].includes(
      this.speciesId,
    );
  }
  ...
  getIconAtlasKey(
    ...
    const isVariant =
      shiny &&
      variantData[variantDataIndex] &&
      variant !== undefined &&
      variantData[variantDataIndex][variant];
  ...
  getIconId(
    ...
    switch (this.speciesId) {
      ...
      case Species.DELTA_IVYSAUR:
      case Species.DELTA_VENUSAUR:
      case Species.DELTA_GARDEVOIR:
        ret += female ? "-f" : "";
        break;
    }
  ...
  validateStarterMoveset(moveset: StarterMoveset, eggMoves: number): boolean {
    ...
    for (const moveId of moveset) {
      if (speciesEggMoves.hasOwnProperty(rootSpeciesId)) {
        const eggMoveIndex = speciesEggMoves[rootSpeciesId].findIndex(
          (m) => m === moveId,
        );
      ...
      if (
        ...
        if (
          !pokemonFormLevelMoves[this.speciesId][this.formIndex].find(
            (lm) => lm[0] <= 5 && lm[1] === moveId,
          )
        ) {
          return false;
        }
      } else if (
        !pokemonSpeciesLevelMoves[this.speciesId].find(
          (lm) => lm[0] <= 5 && lm[1] === moveId,
        )
      ) {
        return false;
  ...
  async loadAssets(
    ...
    globalScene.loadPokemonAtlas(
      spriteKey,
      this.getSpriteAtlasPath(female, formIndex, shiny, variant, back),
    );
    globalScene.load.audio(
      this.getCryKey(formIndex),
      `audio/${this.getCryKey(formIndex)}.m4a`,
    );
    ...
    return new Promise<void>((resolve) => {
      globalScene.load.once(Phaser.Loader.Events.COMPLETE, () => {
        ...
        const spritePath = this.getSpriteAtlasPath(
          female,
          formIndex,
          shiny,
          variant,
          ...
          .replace(/_[1-3]$/, "");
        if (!isNullOrUndefined(variant)) {
          loadPokemonVariantAssets(spriteKey, spritePath, variant).then(() =>
            resolve(),
          );
        }
      });
  ...
  generateCandyColors(): number[][] {
    ...
    if (pixelData?.length !== undefined) {
      for (let i = 0; i < pixelData.length; i += 4) {
        if (pixelData[i + 3]) {
          ...
          const [r, g, b, a] = pixel;
          if (
            !spriteColors.find((c) => c[0] === r && c[1] === g && c[2] === b)
          ) {
    ...
    return Array.from(paletteColors.keys()).map(
      (c) => Object.values(rgbaFromArgb(c)) as number[],
    );
  }
}

export default class PokemonSpecies
  extends PokemonSpeciesForm
  implements Localizable
{
  ...
  getExpandedSpeciesName(): string {
    ...
    return i18next.t(
      `pokemonForm:appendForm.${Species[this.speciesId].split("_")[0]}`,
      { pokemonName: this.name },
    );
  }
  ...
  getFormNameToDisplay(formIndex = 0, append = false): string {
    ...
    const speciesName = capitalizeString(
    ...
    if (this.speciesId === Species.ARCEUS) {
      ...
      if (i18next.exists(i18key)) {
        ...
        const rootSpeciesName = capitalizeString(
          Species[this.getRootSpeciesId()],
          "_",
          true,
          false,
        );
  ...
  getSpeciesForLevel(
    ...
    for (const ev of evolutions) {
      ...
      const isRegionalEvolution =
        !this.isRegional() && evolutionSpecies.isRegional();
        ...
        if (ev.wildDelay === SpeciesWildEvolutionDelay.NONE) {
          if (strength === PartyMemberStrength.STRONGER) {
            ...
            evolutionChance = Math.min(
              minChance +
                easeInFunc(
                  Math.min(level - ev.level, maxLevelDiff) / maxLevelDiff,
                ) *
                  (1 - minChance),
              1,
            );
          ...
          const preferredMinLevel = Math.max(
            ev.level - 1 + ev.wildDelay! * this.getStrengthLevelDiff(strength),
            1,
          ); // TODO: is the bang correct?
          let evolutionLevel = Math.max(
            ev.level > 1 ? ev.level : Math.floor(preferredMinLevel / 2),
            1,
          );

          if (
            ev.level <= 1 &&
            pokemonPrevolutions.hasOwnProperty(this.speciesId)
          ) {
            const prevolutionLevel = pokemonEvolutions[
              pokemonPrevolutions[this.speciesId]
            ].find((ev) => ev.speciesId === this.speciesId)!.level; // TODO: is the bang correct?
          ...
          evolutionChance = Math.min(
            0.65 *
              easeInFunc(
                Math.min(
                  Math.max(level - evolutionLevel, 0),
                  preferredMinLevel,
                ) / preferredMinLevel,
              ) +
              0.35 *
    ...
    if (
      noEvolutionChance === 1 ||
      Phaser.Math.RND.realInRange(0, 1) < noEvolutionChance
    ) {
      return this.speciesId;
    }
  ...
  getEvolutionLevels(): EvolutionLevel[] {
    ...
    if (pokemonEvolutions.hasOwnProperty(this.speciesId)) {
      for (const e of pokemonEvolutions[this.speciesId]) {
        ...
        const nextEvolutionLevels =
          getPokemonSpecies(speciesId).getEvolutionLevels();
    ...
    for (const p of allEvolvingPokemon) {
      for (const e of pokemonEvolutions[p]) {
        if (
          ...
          (!this.forms.length ||
            !e.evoFormKey ||
            e.evoFormKey === this.forms[this.formIndex].formKey) &&
          ...
          const subPrevolutionLevels =
            getPokemonSpecies(speciesId).getPrevolutionLevels();
  ...
  getSimulatedEvolutionChain(
    ...
    if (pokemonPrevolutions.hasOwnProperty(this.speciesId)) {
              ...
              evolution?.level! +
                Math.round(
                  randSeedGauss(0.5, 1 + levelDiff * 0.2) *
                    Math.max(evolution?.wildDelay!, 0.5) *
                    5,
                ) -
                1,
              2,
      ...
      const evolution = pokemonEvolutions[
        prevolutionLevels[prevolutionLevels.length - 1][0]
  ...
  hasVariants() {
    ...
    return (
      ...
      variantData.hasOwnProperty(this.speciesId)
    );
  }
  ...
  getFullUnlocksData(): bigint {
    ...
    caughtAttr +=
      this?.forms?.length > 1
        ? this.forms
            .map((f, index) =>
              f.isUnobtainable ? 0n : 128n * 2n ** BigInt(index),
            )
    ...
    () => {
      while (pokerusStarters.length < POKERUS_STARTER_COUNT) {
        const randomSpeciesId = Number.parseInt(
          randSeedItem(Object.keys(speciesStarterCosts)),
          10,
        );
...
export function initSpecies() {
  allSpecies.push(
    ...
    new PokemonSpecies(
      ...
      0.5,
      9,
      Abilities.SHADOW_CALL,
      Abilities.NONE,
      Abilities.SHADOW_DANCE,
      314,
      44,
      48,
      65,
