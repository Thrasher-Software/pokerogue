...
export class SpeciesFormChange {
  ...
  findTrigger(
    triggerType: Constructor<SpeciesFormChangeTrigger>,
  ): SpeciesFormChangeTrigger | nil {
  ...
  constructor(item: FormChangeItem, active = true) {
    ...
    this.description = this.active
      ? i18next.t("pokemonEvolutions:Forms.item", {
          item: i18next.t(
            `modifierType:FormChangeItem.${FormChangeItem[this.item]}`,
          ),
        })
...
export class SpeciesFormChangeStatusEffectTrigger extends SpeciesFormChangeTrigger {
  ...
  canChange(pokemon: Pokemon): boolean {
    return (
      this.statusEffects.indexOf(pokemon.status?.effect || StatusEffect.NONE) >
        -1 !==
      this.invert
    );
  }
...
export class SpeciesFormChangeMoveLearnedTrigger extends SpeciesFormChangeTrigger {
  ...
  canChange(pokemon: Pokemon): boolean {
    return (
      !!pokemon.moveset.filter((m) => m.moveId === this.move).length ===
      this.known
    );
  }
}
...
export abstract class SpeciesFormChangeMoveTrigger extends SpeciesFormChangeTrigger {
  ...
  constructor(move: Moves | ((m: Moves) => boolean), used = true) {
    super();
    this.movePredicate =
      typeof move === "function" ? move : (m: Moves) => m === move;
    this.used = used;
  }
}
...
export class SpeciesFormChangePreMoveTrigger extends SpeciesFormChangeMoveTrigger {
  ...
  canChange(pokemon: Pokemon): boolean {
    ...
    return (
      !!command?.move && this.movePredicate(command.move.move) === this.used
    );
  }
}
...
export class SpeciesFormChangePostMoveTrigger extends SpeciesFormChangeMoveTrigger {
  ...
  canChange(pokemon: Pokemon): boolean {
    return (
      pokemon.summonData &&
      !!pokemon.getLastXMoves(1).filter((m) => this.movePredicate(m.move))
        .length === this.used
    );
  }
}
...
export class SpeciesFormChangeRevertWeatherFormTrigger extends SpeciesFormChangeTrigger {
  ...
  canChange(pokemon: Pokemon): boolean {
    if (pokemon.hasAbility(this.ability, false, true)) {
      ...
      const isAbilityChanged =
        summonDataAbility !== this.ability &&
        summonDataAbility !== Abilities.NONE;
...
export function getSpeciesFormChangeMessage(
  pokemon: Pokemon,
  ...
  const isRevert =
    !isMega && formChange.formKey === pokemon.species.forms[0].formKey;
  if (isMega) {
 ...
 */
function getSpeciesDependentFormChangeCondition(
  species: Species,
): SpeciesFormChangeCondition {
  return new SpeciesFormChangeCondition(
    (_p) => !!globalScene.gameData.dexData[species].caughtAttr,
  );
}
...
export const pokemonFormChanges: PokemonFormChanges = {
  ...
  [Species.MEWTWO]: [
    new SpeciesFormChange(
      Species.MEWTWO,
      "",
      SpeciesFormKey.ARMORED,
      new SpeciesFormChangeItemTrigger(FormChangeItem.MEWTWO_ARMOR),
    ),
    new SpeciesFormChange(
      Species.MEWTWO,
      "",
  ...
  [Species.CASTFORM]: [
    new SpeciesFormChange(
      ...
      "",
      "sunny",
      new SpeciesFormChangeWeatherTrigger(Abilities.FORECAST, [
        WeatherType.SUNNY,
    ...
    new SpeciesFormChange(
      ...
      "sunny",
      "",
      new SpeciesFormChangeActiveTrigger(),
      true,
    ),
    new SpeciesFormChange(
      Species.CASTFORM,
  ...
  [Species.CHERRIM]: [
    new SpeciesFormChange(
      ...
      "overcast",
      "sunshine",
      new SpeciesFormChangeWeatherTrigger(Abilities.FLOWER_GIFT, [
        WeatherType.SUNNY,
        WeatherType.HARSH_SUN,
      ]),
      true,
  ...
  [Species.SHAYMIN]: [
    new SpeciesFormChange(
      ...
      "land",
      "sky",
      new SpeciesFormChangeItemTrigger(FormChangeItem.GRACIDEA),
    ),
  ],
  [Species.ARCEUS]: [
    new SpeciesFormChange(
      ...
      true,
      new SpeciesFormChangeCondition((p) => p.hasAbility(Abilities.MULTITYPE)),
    ),
    new SpeciesFormChange(
      Species.ARCEUS,
      "normal",
  ...
  [Species.KELDEO]: [
    new SpeciesFormChange(
      ...
      false,
      new SpeciesFormChangeCondition(
  ...
  [Species.GENESECT]: [
    ...
    new SpeciesFormChange(
      ...
      "",
      "burn",
      new SpeciesFormChangeItemTrigger(FormChangeItem.BURN_DRIVE),
    ),
    new SpeciesFormChange(
      Species.GENESECT,
      "",
  ...
  [Species.AEGISLASH]: [
    new SpeciesFormChange(
      ...
      true,
      new SpeciesFormChangeCondition((p) =>
        p.hasAbility(Abilities.STANCE_CHANGE),
      ),
    ),
    new SpeciesFormChange(
      Species.AEGISLASH,
      "shield",
      "blade",
      new SpeciesFormChangePreMoveTrigger(
        (m) => allMoves[m].category !== MoveCategory.STATUS,
      ),
      true,
      new SpeciesFormChangeCondition((p) =>
        p.hasAbility(Abilities.STANCE_CHANGE),
    ...
    new SpeciesFormChange(
      ...
      "blade",
      "shield",
      new SpeciesFormChangeActiveTrigger(false),
      true,
    ),
  ],
  [Species.XERNEAS]: [
    new SpeciesFormChange(
      ...
      "neutral",
      "active",
      new SpeciesFormChangeActiveTrigger(true),
      true,
    ),
    new SpeciesFormChange(
      Species.XERNEAS,
  ...
  [Species.SILVALLY]: [
    new SpeciesFormChange(
      ...
      true,
      new SpeciesFormChangeCondition((p) => p.hasAbility(Abilities.RKS_SYSTEM)),
    ),
    new SpeciesFormChange(
      Species.SILVALLY,
      "normal",
  ...
  [Species.DELTA_SCIZOR]: [
    new SpeciesFormChange(
      ...
      "",
      SpeciesFormKey.MEGA,
      new SpeciesFormChangeItemTrigger(FormChangeItem.DELTA_SCIZORITE),
    ),
  ],
  [Species.DELTA_PIDGEOT]: [
    new SpeciesFormChange(
      ...
      "",
      SpeciesFormKey.MEGA,
      new SpeciesFormChangeItemTrigger(FormChangeItem.DELTA_PIDGEOTITE),
    ),
  ],
...
export function initPokemonForms() {
  ...
  for (const pk of formChangeKeys) {
    ...
    for (const fc of formChanges) {
      const itemTrigger = fc.findTrigger(
        SpeciesFormChangeItemTrigger,
      ) as SpeciesFormChangeItemTrigger;
      if (
        itemTrigger &&
        !formChanges.find(
          (c) => fc.formKey === c.preFormKey && fc.preFormKey === c.formKey,
        )
      ) {
        newFormChanges.push(
          new SpeciesFormChange(
