...
export default class Move implements Localizable {
  ...
  localize(): void {
    ...
    this.name = this.id
      ? `${i18next.t(`move:${i18nKey}.name`)}${this.nameAppend}`
      : "";
    this.effect = this.id
   ...
   */
  attr<T extends Constructor<MoveAttr>>(
    AttrType: T,
    ...
    switch (flag) {
      case MoveFlags.MAKES_CONTACT:
        if (
          ...
          this.hitsSubstitute(user, target)
        ) {
          return false;
        }
        break;
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    ...
    if (
      target.getAlly()?.getTag(BattlerTagType.COMMANDED)?.getSourcePokemon() ===
      target
    ) {
    ...
    for (const attr of this.attrs) {
      ...
      score +=
        attr.getTargetBenefitScore(
          user,
          !attr.selfTarget ? target : user,
          move,
  ...
  calculateBattlePower(
    ...
    if (
      ...
      !globalScene.findModifier(
        (m) =>
          m instanceof PokemonMultiHitModifier && m.pokemonId === source.id,
      )
    ) {
      power.value = 60;
    }
    ...
    const alliedField: Pokemon[] =
      source instanceof PlayerPokemon
    ...
    const typeBoost = source.findTag(
      (t) =>
        t instanceof TypeBoostTag && t.boostedType === typeChangeHolder.value,
    ) as TypeBoostTag;
    if (typeBoost) {
  ...
  calculateEffectivePower(): number {
    ...
    if (this.id === Moves.TRIPLE_AXEL) {
      ...
      if (multiHitAttr) {
        effectivePower =
          multiHitAttr.calculateExpectedHitCount(this) * this.power;
      } else {
        effectivePower =
          this.power * (this.accuracy === -1 ? 1 : this.accuracy / 100);
      }
    }
  ...
  canBeMultiStrikeEnhanced(
    ...
    const exceptPollenPuffAlly: boolean =
      this.id === Moves.POLLEN_PUFF &&
      !isNullOrUndefined(ally) &&
      targets.includes(ally.getBattlerIndex());
...
export class AttackMove extends Move {
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    ...
    const statHolder = new NumberHolder(
      user.getEffectiveStat(thisStat, target),
    );
...
function ChargeMove<TBase extends SubMove>(Base: TBase) {
  return class extends Base {
    ...
    public readonly chargeAnim: ChargeAnim =
      ChargeAnim[`${Moves[this.id]}_CHARGING`];
    ...
    getChargeAttrs<T extends MoveAttr>(attrType: Constructor<T>): T[] {
      return this.chargeAttrs.filter(
        (attr): attr is T => attr instanceof attrType,
      );
    }
     ...
     * @returns this {@linkcode Move} (for chaining API purposes)
     */
    chargeAttr<T extends Constructor<MoveAttr>>(
      ChargeAttrType: T,
      ...args: ConstructorParameters<T>
    ): this {
...
export class MoveEffectAttr extends MoveAttr {
  ...
  canApply(user: Pokemon, target: Pokemon, move: Move, args?: any[]) {
    return (
      !!(this.selfTarget
        ? user.hp && !user.getTag(BattlerTagType.FRENZY)
        : target.hp) &&
      (this.selfTarget ||
        !target.getTag(BattlerTagType.PROTECTED) ||
        move.doesFlagEffectApply({
          flag: MoveFlags.IGNORE_PROTECT,
          user,
          target,
        }))
    );
  ...
  getMoveChance(
    ...
    const moveChance = new NumberHolder(
    ...
    if (
      (!move.hasAttr(FlinchAttr) || moveChance.value <= move.chance) &&
      !move.hasAttr(SecretPowerAttr)
    ) {
      const userSide = user.isPlayer()
        ? ArenaTagSide.PLAYER
        : ArenaTagSide.ENEMY;
...
export class PreMoveMessageAttr extends MoveAttr {
  ...
  constructor(
    message: string | ((user: Pokemon, target: Pokemon, move: Move) => string),
  ) {
    super();
    this.message = message;
  }
...
export class TargetHalfHpDamageAttr extends FixedDamageAttr {
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    return target.getHpRatio() > 0.5
      ? Math.floor((target.getHpRatio() - 0.5) * -24 + 4)
      : -20;
  }
}
...
export class CounterDamageAttr extends FixedDamageAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      !!user.turnData.attacksReceived.filter((ar) =>
        this.moveFilter(allMoves[ar.move]),
      ).length;
  }
}
...
export class RandomLevelDamageAttr extends FixedDamageAttr {
  ...
  getDamage(user: Pokemon, target: Pokemon, move: Move): number {
    return toDmgValue(
      user.level * (user.randBattleSeedIntRange(50, 150) * 0.01),
    );
  }
}
...
export class ModifiedDamageAttr extends MoveAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    initialDamage.value = this.getModifiedDamage(
      user,
      target,
      move,
      initialDamage.value,
...
export class CelebrateAttr extends MoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    globalScene.queueMessage(
      i18next.t("moveTriggers:celebrate", {
...
export class RecoilAttr extends MoveEffectAttr {
  ...
  constructor(
    useHp: boolean = false,
    damageRatio: number = 0.25,
    unblockable: boolean = false,
  ) {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      this.useHp &&
      [MoveResult.FAIL, MoveResult.MISS].includes(
        user.getLastXMoves(1)[0]?.result ?? MoveResult.FAIL,
      )
    ) {
      return false;
    }
    ...
    const damageValue =
      (!this.useHp ? user.turnData.totalDamageDealt : user.getMaxHp()) *
      this.damageRatio;
...
export class SacrificialAttr extends MoveEffectAttr {
  ...
  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    ...
    return Math.ceil(
      ((1 - user.getHpRatio()) * 10 - 10) *
        (target.getAttackTypeEffectiveness(move.type, user) - 0.5),
    );
  }
}
...
export class HalfSacrificialAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!cancelled.value) {
      ...
      globalScene.queueMessage(
        i18next.t("moveTriggers:cutHpPowerUpMove", {
          pokemonName: getPokemonNameWithAffix(user),
        }),
...
export class AddSubstituteAttr extends MoveEffectAttr {
  ...
  getFailedText(
    ...
    if (user.getTag(SubstituteTag)) {
      return i18next.t("moveTriggers:substituteOnOverlap", {
        pokemonName: getPokemonNameWithAffix(user),
      });
    } else if (
      user.hp <= Math.floor(user.getMaxHp() / 4) ||
      user.getMaxHp() === 1
    ) {
...
export class HealAttr extends MoveEffectAttr {
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    const score =
      (1 - (this.selfTarget ? user : target).getHpRatio()) * 20 -
      this.healRatio * 10;
...
export class PartyStatusCureAttr extends MoveEffectAttr {
  ...
  canApply(user: Pokemon, target: Pokemon, move: Move, args: any[]) {
    const isTargetValid =
      ...
      (!this.selfTarget &&
        (!target.getTag(BattlerTagType.PROTECTED) ||
          move.hasFlag(MoveFlags.IGNORE_PROTECT)));
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const partyPokemon = user.isPlayer()
...
export class FlameBurstAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    targetAlly.damageAndUpdate(
      Math.max(1, Math.floor((1 / 16) * targetAlly.getMaxHp())),
      { result: HitResult.INDIRECT },
    );
    return true;
  }
...
export class SacrificialFullRestoreAttr extends SacrificialAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, _target, _move) =>
      globalScene.getPlayerParty().filter((p) => p.isActive()).length >
...
export class BoostHealAttr extends HealAttr {
   ...
   */
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const healRatio: number = (
      this.condition ? this.condition(user, target, move) : false
    )
...
export class HitHealAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (this.healStat !== null) {
      ...
      message = i18next.t("battle:drainMessage", {
        pokemonName: getPokemonNameWithAffix(target),
      });
    } else {
      // Default healing formula used by draining moves like Absorb, Draining Kiss, Bitter Blade, etc.
      healAmount = toDmgValue(
        user.turnData.singleHitDamageDealt * this.healRatio,
      );
      message = i18next.t("battle:regainHealth", {
        pokemonName: getPokemonNameWithAffix(user),
      });
    }
    if (reverseDrain) {
  ...
  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    if (this.healStat) {
      ...
      return Math.floor(
        Math.max(
          0,
          Math.min(1, (healAmount + user.hp) / user.getMaxHp() - 0.33),
        ) / user.getHpRatio(),
      );
    }
    return Math.floor(
      Math.max(1 - user.getHpRatio() - 0.33, 0) * (move.power / 4),
    );
  }
}
...
export class MultiHitAttr extends MoveAttr {
  ...
  constructor(multiHitType?: MultiHitType) {
    ...
    this.intrinsicMultiHitType =
      multiHitType !== undefined ? multiHitType : MultiHitType._2_TO_5;
      ...
      case MultiHitType.BEAT_UP:
        ...
        return party.reduce((total, pokemon) => {
          return (
            ...
            (pokemon.id === user.id
              ? 1
              : pokemon?.status && pokemon.status.effect !== StatusEffect.NONE
                ? 0
                : 1)
          );
        }, 0);
...
export class WaterShurikenMultiHitTypeAttr extends ChangeMultiHitTypeAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (
      user.species.speciesId === Species.GRENINJA &&
      user.hasAbility(Abilities.BATTLE_BOND) &&
      user.formIndex === 2
    ) {
...
export class StatusEffectAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (statusCheck) {
      ...
      if (
        ...
        !target.canSetStatus(this.effect, quiet, false, user, true)
      ) {
        return false;
      }
      if (
        (!pokemon.status ||
          this.overrideStatus ||
          (pokemon.status.effect === this.effect && moveChance < 0)) &&
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    ...
    return !pokemon.status &&
      pokemon.canSetStatus(this.effect, true, false, user)
      ? score
      : 0;
  }
}
...
export class PsychoShiftEffectAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, _move: Move, _args: any[]): boolean {
    ...
    if (target.status) {
      ...
      const canSetStatus = target.canSetStatus(
        statusToApply,
        true,
        false,
        user,
      );
      const trySetStatus = canSetStatus
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    return !target.status &&
      target.canSetStatus(user.status?.effect, true, false, user)
      ? -10
      : 0;
  }
}
...
export class StealHeldItemChanceAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const heldItems = this.getTargetHeldItems(target).filter(
      (i) => i.isTransferable,
    );
    if (!heldItems.length) {
      return false;
    ...
    const tierHeldItems = heldItems.filter(
      (m) => m.type.getOrInferTier(poolType) === highestItemTier,
    );
    const stolenItem =
      tierHeldItems[user.randBattleSeedInt(tierHeldItems.length)];
...
export class RemoveHeldItemAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    let heldItems = this.getTargetHeldItems(target).filter(
      (i) => i.isTransferable,
    );

    if (this.berriesOnly) {
      heldItems = heldItems.filter(
...
export class EatBerryAttr extends MoveEffectAttr {
  ...
  getTargetHeldBerries(target: Pokemon): BerryModifier[] {
    return globalScene.findModifiers(
      (m) =>
        ...
        (m as BerryModifier).pokemonId === target.id,
      target.isPlayer(),
    ) as BerryModifier[];
  }
  ...
  protected eatBerry(
    consumer: Pokemon,
    berryOwner: Pokemon = consumer,
    updateHarvest = consumer === berryOwner,
  ) {
...
export class TerrainChangeAttr extends MoveEffectAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      !globalScene.arena.terrain ||
      globalScene.arena.terrain.terrainType !== this.terrainType;
  }
  ...
  constructor(weatherTypes: WeatherType[]) {
    super((user, move) => {
      ...
      if (isNullOrUndefined(currentWeather?.weatherType)) {
        ...
        return (
          !currentWeather?.isEffectSuppressed() &&
...
export class DelayedAttackAttr extends OverrideMoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (!virtual) {
      ...
      globalScene.unshiftPhase(
        new MoveAnimPhase(new MoveChargeAnim(this.chargeAnim, move.id, user)),
      );
      globalScene.queueMessage(
        this.chargeText
          .replace("{TARGET}", getPokemonNameWithAffix(target))
      ...
      globalScene.queueMessage(
        i18next.t("moveTriggers:tookMoveAttack", {
          pokemonName: getPokemonNameWithAffix(
            globalScene.getPokemonById(target.id) ?? undefined,
          ),
          moveName: move.name,
        }),
      );
...
export class AwaitCombinedPledgeAttr extends OverrideMoveEffectAttr {
  ...
  override apply(
    ...
    const allyMovePhase = globalScene.findPhase<MovePhase>(
      (phase) =>
        phase instanceof MovePhase &&
        phase.pokemon.isPlayer() === user.isPlayer(),
    );
    if (allyMovePhase) {
      const allyMove = allyMovePhase.move.getMove();
      if (allyMove !== move && allyMove.hasAttr(AwaitCombinedPledgeAttr)) {
        [user, allyMovePhase.pokemon].forEach(
          (p) => (p.turnData.combiningPledge = move.id),
        );
        ...
        const allyMovePhaseIndex =
          globalScene.phaseQueue.indexOf(allyMovePhase);
        ...
        if (allyMovePhaseIndex !== firstMovePhaseIndex) {
          globalScene.prependToPhase(
            globalScene.phaseQueue.splice(allyMovePhaseIndex, 1)[0],
            MovePhase,
          );
        }
...
export class StatStageChangeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args?: any[]): boolean {
    if (
      !super.apply(user, target, move, args) ||
      (this.condition && !this.condition(user, target, move))
    ) {
      return false;
    }
...
export class SecretPowerAttr extends MoveEffectAttr {
  ...
  private determineBiomeEffect(biome: Biome): MoveEffectAttr {
    ...
    switch (biome) {
      ...
      case Biome.ABYSS:
      case Biome.SPACE:
        secondaryEffect = new AddBattlerTagAttr(
...
export class PostVictoryStatStageChangeAttr extends MoveAttr {
  ...
  applyPostVictory(user: Pokemon, target: Pokemon, move: Move): void {
    ...
    const statChangeAttr = new StatStageChangeAttr(
      this.stats,
      this.stages,
      this.showMessage,
    );
...
export class CutHpStatStageBoostAttr extends StatStageChangeAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, _target, _move) =>
      user.getHpRatio() > 1 / this.cutRatio &&
...
export class InvertStatsAttr extends MoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    globalScene.queueMessage(
      i18next.t("moveTriggers:invertStats", {
        pokemonName: getPokemonNameWithAffix(target),
      }),
    );
...
export class ResetStatsAttr extends MoveEffectAttr {
  ...
  override apply(
    ...
    if (this.targetAllPokemon) {
      ...
      globalScene.queueMessage(
        i18next.t("moveTriggers:resetStats", {
          pokemonName: getPokemonNameWithAffix(target),
        }),
      );
    }
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (super.apply(user, target, move, args)) {
      ...
      if (this.stats.length === 7) {
        globalScene.queueMessage(
          i18next.t("moveTriggers:switchedStatChanges", {
            pokemonName: getPokemonNameWithAffix(user),
          }),
        );
...
export class MovePowerMultiplierAttr extends VariablePowerAttr {
  ...
  constructor(
    powerMultiplier: (user: Pokemon, target: Pokemon, move: Move) => number,
  ) {
    super();
...
export class BeatUpAttr extends VariablePowerAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const allyIndex =
      (user.turnData.hitCount - user.turnData.hitsLeft) % allyCount;
    power.value = beatUpFunc(user, allyIndex);
    return true;
  }
}

const doublePowerChanceMessageFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) => {
  ...
  globalScene.executeWithSeedOffset(
    () => {
      ...
      if (rand < move.chance) {
        message = i18next.t("moveTriggers:goingAllOutForAttack", {
          pokemonName: getPokemonNameWithAffix(user),
        });
      }
    },
    ...
    super((user: Pokemon, target: Pokemon, move: Move): number => {
      ...
      while (
        ((turnMove = moveHistory.shift())?.move === move.id ||
          (comboMoves.length &&
            comboMoves.includes(turnMove?.move ?? Moves.NONE))) &&
...
export class ElectroBallPowerAttr extends VariablePowerAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const statRatio =
      target.getEffectiveStat(Stat.SPD) / user.getEffectiveStat(Stat.SPD);
...
export class GyroBallPowerAttr extends VariablePowerAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    power.value = Math.floor(
      Math.min(150, (25 * target.getEffectiveStat(Stat.SPD)) / userSpeed + 1),
    );
    return true;
  }
}
...
export class OpponentHighHpPowerAttr extends VariablePowerAttr {
   ...
   */
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    (args[0] as NumberHolder).value = toDmgValue(
      this.maxBasePower * target.getHpRatio(),
    );
...
export class FirstAttackDoublePowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      !target
        .getLastXMoves(1)
        .find((m) => m.turn === globalScene.currentBattle.turn)
    ) {
...
export class TurnDamagedDoublePowerAttr extends VariablePowerAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (
      user.turnData.attacksReceived.find(
        (r) => r.damage && r.sourceId === target.id,
      )
    ) {
      (args[0] as NumberHolder).value *= 2;
      return true;
...
const magnitudeMessageFunc = (user: Pokemon, target: Pokemon, move: Move) => {
  ...
  globalScene.executeWithSeedOffset(
    () => {
      ...
      message = i18next.t("moveTriggers:magnitudeMessage", {
        magnitude: m + 4,
      });
    },
    globalScene.currentBattle.turn << 6,
    ...
    const friendshipPower = Math.floor(
      Math.min(
        user instanceof PlayerPokemon
          ? user.friendship
          : user.species.baseFriendship,
        255,
      ) / 2.5,
    );
    power.value = Math.max(
      !this.invert ? friendshipPower : 102 - friendshipPower,
      1,
    );
...
const countPositiveStatStages = (pokemon: Pokemon): number => {
  return pokemon
    .getStatStages()
    .reduce((total, stat) => (stat && stat > 0 ? total + stat : total), 0);
};
    ...
    if (powerSeed <= 40) {
      ...
      globalScene.unshiftPhase(
        new PokemonHealPhase(
          ...
          i18next.t("moveTriggers:regainedHealth", {
            pokemonName: getPokemonNameWithAffix(target),
          }),
          true,
        ),
...
export class MultiHitPowerIncrementAttr extends VariablePowerAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const hitsTotal =
      user.turnData.hitCount - Math.max(user.turnData.hitsLeft, 0);
    const power = args[0] as NumberHolder;
...
export class CueNextRoundAttr extends MoveEffectAttr {
  ...
  override apply(
    ...
    const nextMoveIndex = globalScene.phaseQueue.findIndex(
      (phase) => phase instanceof MovePhase,
    );
    if (nextRoundIndex !== nextMoveIndex) {
      globalScene.prependToPhase(
        globalScene.phaseQueue.splice(nextRoundIndex, 1)[0],
        MovePhase,
      );
    }
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    for (const s of BATTLE_STATS) {
      ...
      if (statStageValueTarget > 0) {
         ...
         */
        const availableToSteal = Math.min(
          statStageValueTarget,
          6 - statStageValueUser,
        );
...
export class PhotonGeyserCategoryAttr extends VariableMoveCategoryAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      user.getEffectiveStat(Stat.ATK, target, move) >
      user.getEffectiveStat(Stat.SPATK, target, move)
    ) {
...
export class FormChangeItemTypeAttr extends VariableMoveTypeAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      [user.species.speciesId, user.fusionSpecies?.speciesId].includes(
        Species.ARCEUS,
      ) ||
      [user.species.speciesId, user.fusionSpecies?.speciesId].includes(
        Species.SILVALLY,
      ...
      const form =
        user.species.speciesId === Species.ARCEUS ||
        user.species.speciesId === Species.SILVALLY
          ? user.formIndex
...
export class TechnoBlastTypeAttr extends VariableMoveTypeAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      ...
      const form =
        user.species.speciesId === Species.GENESECT
          ? user.formIndex
          : user.fusionSpecies?.formIndex;
...
export class AuraWheelTypeAttr extends VariableMoveTypeAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      ...
      const form =
        user.species.speciesId === Species.MORPEKO
          ? user.formIndex
          : user.fusionSpecies?.formIndex;
...
export class RagingBullTypeAttr extends VariableMoveTypeAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      ...
      const form =
        user.species.speciesId === Species.PALDEA_TAUROS
          ? user.formIndex
          : user.fusionSpecies?.formIndex;
...
export class IvyCudgelTypeAttr extends VariableMoveTypeAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      ...
      const form =
        user.species.speciesId === Species.OGERPON
          ? user.formIndex
          : user.fusionSpecies?.formIndex;
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (user.level < target.level) {
      ...
      accuracy.value = Math.min(
        Math.max(30 + 100 * (1 - target.level / user.level), 0),
        100,
      );
    }
    return true;
...
export class SheerColdAccuracyAttr extends OneHitKOAccuracyAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (user.level < target.level) {
      ...
      accuracy.value = Math.min(
        Math.max(baseAccuracy + 100 * (1 - target.level / user.level), 0),
        100,
      );
    }
    return true;
...
const crashDamageFunc = (user: Pokemon, move: Move) => {
  ...
  globalScene.queueMessage(
    i18next.t("moveTriggers:keptGoingAndCrashed", {
      pokemonName: getPokemonNameWithAffix(user),
    }),
  );
...
export const frenzyMissFunc: UserMoveConditionFunc = (
  ...
  while (
    user.getMoveQueue().length &&
    user.getMoveQueue()[0].move === move.id
  ) {
...
export class AddBattlerTagAttr extends MoveEffectAttr {
  ...
  constructor(
    ...
    this.turnCountMax =
      turnCountMax !== undefined ? turnCountMax : turnCountMin;
  ...
  getCondition(): MoveConditionFunc | null {
    return this.failOnOverlap
      ? (user, target, move) =>
          !(this.selfTarget ? user : target).getTag(this.tagType)
      : null;
  }
  ...
  getTagTargetBenefitScore(): number {
    switch (this.tagType) {
      ...
      default:
        console.warn(
          `BattlerTag ${BattlerTagType[this.tagType]} is missing a score!`,
        );
        return 0;
    }
  }
  ...
  getTargetBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    let moveChance = this.getMoveChance(
      user,
      target,
      move,
      this.selfTarget,
...
export class FallDownAttr extends AddBattlerTagAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (!target.isGrounded()) {
      globalScene.queueMessage(
        i18next.t("moveTriggers:fallDown", {
...
export class GulpMissileTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, _target: Pokemon, move: Move, _args: any[]): boolean {
    ...
    if (
      user.hasAbility(Abilities.GULP_MISSILE) &&
...
export class CurseAttr extends MoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    if (user.getTypes(true).includes(PokemonType.GHOST)) {
      ...
      globalScene.unshiftPhase(
        new StatStageChangePhase(user.getBattlerIndex(), true, [Stat.SPD], -1),
      );
      return true;
    }
  }
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move): boolean => {
      ...
      while (moveHistory.length) {
        ...
        if (
          !allMoves[turnMove?.move ?? Moves.NONE].hasAttr(ProtectAttr) ||
    ...
    globalScene
      ...
      .forEach((pokemon) =>
        pokemon.findAndRemoveTags(
          (tag) => tag.tagType === BattlerTagType.SUBSTITUTE,
        ),
      );
    return true;
  }
...
export class AddArenaTagAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      (move.chance < 0 ||
        move.chance === 100 ||
        user.randBattleSeedInt(100) < move.chance) &&
      user.getLastXMoves(1)[0]?.result === MoveResult.SUCCESS
    ) {
      const side =
        (this.selfSideTarget ? user : target).isPlayer() !==
        (move.hasAttr(AddArenaTrapTagAttr) && target === user)
          ? ArenaTagSide.PLAYER
          : ArenaTagSide.ENEMY;
      globalScene.arena.addTag(
        this.tagType,
...
export class RemoveArenaTagsAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const side = (this.selfSideTarget ? user : target).isPlayer()
      ? ArenaTagSide.PLAYER
      : ArenaTagSide.ENEMY;
...
export class AddArenaTrapTagAttr extends AddArenaTagAttr {
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      const side =
        this.selfSideTarget !== user.isPlayer()
          ? ArenaTagSide.ENEMY
          : ArenaTagSide.PLAYER;
...
export class SwapArenaTagsAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    globalScene.queueMessage(
      i18next.t("moveTriggers:swapArenaTags", {
        pokemonName: getPokemonNameWithAffix(user),
      }),
    );
    return true;
...
export class RevivalBlessingAttr extends MoveEffectAttr {
  ...
  override apply(
    ...
    if (user instanceof PlayerPokemon) {
      ...
      globalScene
        .getEnemyParty()
        .findIndex((p) => p.isFainted() && !p.isBoss()) > -1
    ) {
      ...
      const faintedPokemon = globalScene
        .getEnemyParty()
        .filter((p) => p.isFainted() && !p.isBoss());
      const pokemon =
        faintedPokemon[user.randBattleSeedInt(faintedPokemon.length)];
      const slotIndex = globalScene
        .getEnemyParty()
        .findIndex((p) => pokemon.id === p.id);
      pokemon.resetStatus(true, false, false, true);
      pokemon.heal(
        Math.min(toDmgValue(0.5 * pokemon.getMaxHp()), pokemon.getMaxHp()),
      );
      globalScene.queueMessage(
        ...
        if (allyPokemon.isFainted() || allyPokemon === pokemon) {
          ...
          globalScene.tryRemovePhase(
            (phase: SwitchSummonPhase) =>
              phase instanceof SwitchSummonPhase &&
              phase.getPokemon() === pokemon,
          );
          // If the pokemon being revived was alive earlier in the turn, cancel its move
          // (revived pokemon can't move in the turn they're brought back)
          globalScene
            .findPhase((phase: MovePhase) => phase.pokemon === pokemon)
            ?.cancel();
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      (user instanceof PlayerPokemon &&
        globalScene.getPlayerParty().some((p) => p.isFainted())) ||
      (user instanceof EnemyPokemon &&
        user.hasTrainer() &&
        globalScene.getEnemyParty().some((p) => p.isFainted() && !p.isBoss()));
  }

  override getUserBenefitScore(
    user: Pokemon,
    _target: Pokemon,
    _move: Move,
  ): number {
...
export class ForceSwitchOutAttr extends MoveEffectAttr {
    ...
    if (switchOutTarget instanceof PlayerPokemon) {
      ...
      if (switchOutTarget.hp > 0) {
        if (this.switchType === SwitchType.FORCE_SWITCH) {
          ...
          const slotIndex =
            eligibleNewIndices[
              user.randBattleSeedInt(eligibleNewIndices.length)
            ];
          globalScene.prependToPhase(
            new SwitchSummonPhase(
              this.switchType,
      ...
      if (switchOutTarget.hp > 0) {
        ...
        globalScene.queueMessage(
          i18next.t("moveTriggers:fled", {
            pokemonName: getPokemonNameWithAffix(switchOutTarget),
          }),
          null,
          true,
          500,
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      move.category !== MoveCategory.STATUS ||
      this.getSwitchOutCondition()(user, target, move);
  }
  ...
  getFailedText(
    ...
    if (blockedByAbility.value) {
      return i18next.t("moveTriggers:cannotBeSwitchedOut", {
        pokemonName: getPokemonNameWithAffix(target),
      });
    }
  }
    ...
    return (user, target, move) => {
      ...
      if (!this.selfSwitch) {
        ...
        if (forceSwitchAttr && globalScene.currentBattle.trainer) {
          ...
          const remainingPokemon = enemyParty.filter(
            (p) => p.hp > 0 && p.isAllowedInBattle(),
          );
  ...
  getUserBenefitScore(user: Pokemon, target: Pokemon, move: Move): number {
    if (
      !globalScene.getEnemyParty().find((p) => p.isActive() && !p.isOnField())
    ) {
      return -20;
    }
    ...
    if (this.selfSwitch && this.isBatonPass()) {
      const statStageTotal = user
        .getStatStages()
        .reduce((s: number, total: number) => (total += s), 0);
      ret =
        ret / 2 +
        Phaser.Tweens.Builders.GetEaseFunction("Sine.easeOut")(
          Math.min(Math.abs(statStageTotal), 10) / 10,
        ) *
  ...
  hpDroppedBelowHalf(target: Pokemon): boolean {
    ...
    return (
      initialHealth >= maxPokemonHealth / 2 &&
      pokemonHealth < maxPokemonHealth / 2
    );
  }
}
...
export class ChillyReceptionAttr extends ForceSwitchOutAttr {
  ...
  getCondition(): MoveConditionFunc {
    ...
    return (user, target, move) =>
      ...
      super.getSwitchOutCondition()(user, target, move);
  }
}
export class RemoveTypeAttr extends MoveEffectAttr {
  ...
  constructor(
    removedType: PokemonType,
    messageCallback?: (user: Pokemon) => void,
  ) {
    super(true, { trigger: MoveEffectTrigger.POST_TARGET });
...
export class CopyTypeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      targetTypes.includes(PokemonType.UNKNOWN) &&
      targetTypes.indexOf(PokemonType.UNKNOWN) > -1
    ) {
      targetTypes[targetTypes.indexOf(PokemonType.UNKNOWN)] =
        PokemonType.NORMAL;
    }
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      target.getTypes()[0] !== PokemonType.UNKNOWN ||
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    globalScene.queueMessage(
      i18next.t("moveTriggers:transformedIntoType", {
        pokemonName: getPokemonNameWithAffix(user),
        typeName: i18next.t(`pokemonInfo:Type.${PokemonType[typeChange]}`),
      }),
    );
...
export class AddTypeAttr extends MoveEffectAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      !target.isTerastallized && !target.getTypes().includes(this.type);
  }
}
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    globalScene.queueMessage(
      i18next.t("battle:transformedIntoType", {
        pokemonName: getPokemonNameWithAffix(user),
        type: i18next.t(`pokemonInfo:Type.${PokemonType[firstMoveType]}`),
      }),
    );
      ...
      moveTargets.multiple || moveTargets.targets.length === 1
        ...
        : [
            this.hasTarget
              ? target.getBattlerIndex()
              : moveTargets.targets[
                  user.randBattleSeedInt(moveTargets.targets.length)
                ],
          ]; // account for Mirror Move having a target already
    user
      .getMoveQueue()
      .push({ move: move.id, targets: targets, virtual: true, ignorePP: true });
    globalScene.unshiftPhase(new LoadMoveAnimPhase(move.id));
    globalScene.unshiftPhase(
      new MovePhase(
        user,
        targets,
        new PokemonMove(move.id, 0, 0, true),
        true,
        true,
      ),
    );
...
export class RandomMoveAttr extends CallMoveAttr {
  ...
  override apply(
    ...
    const moveIds = getEnumValues(Moves).map((m) =>
      !this.invalidMoves.has(m) && !allMoves[m].name.endsWith(" (N)")
        ? m
        : Moves.NONE,
    );
    ...
    do {
      moveId =
        this.getMoveOverride() ??
        moveIds[user.randBattleSeedInt(moveIds.length)];
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      ...
      const moves = partyMoveset.filter(
        (m) =>
          !this.invalidMoves.has(m!.moveId) &&
          !m!.getMove().name.endsWith(" (N)"),
      );
      if (moves.length === 0) {
        return false;
      }
...
export class CopyMoveAttr extends CallMoveAttr {
  ...
  constructor(
    mirrorMove: boolean,
    invalidMoves: ReadonlySet<Moves> = new Set(),
  ) {
    super();
...
export class RepeatMoveAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const lastMove = target
      .getLastXMoves(-1)
      .find((m) => m.move !== Moves.NONE)!;
    const movesetMove = target
      .getMoveset()
      .find((m) => m.moveId === lastMove.move)!;
    ...
    let moveTargets = movesetMove.getMove().isMultiTarget()
      ? getMoveTargets(target, lastMove.move).targets
      : lastMove.targets;
    ...
    */
    ...
    target
      .getMoveQueue()
      .unshift({ move: lastMove.move, targets: moveTargets, ignorePP: false });
    target.turnData.extraTurns++;
    globalScene.appendToPhase(
      new MovePhase(target, moveTargets, movesetMove),
      MoveEndPhase,
    );
    return true;
  }
...
export class ReducePpMoveAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    movesetMove.ppUsed = Math.min(
      lastPpUsed + this.reduction,
      movesetMove.getMovePp(),
    );
    ...
    globalScene.eventTarget.dispatchEvent(
      new MoveUsedEvent(target.id, movesetMove.getMove(), movesetMove.ppUsed),
    );
    globalScene.queueMessage(message);
...
const targetMoveCopiableCondition: MoveConditionFunc = (user, target, move) => {
  ...
  if (
    allMoves[copiableMove.move].isChargingMove() &&
    copiableMove.result === MoveResult.OTHER
  ) {
    return false;
  }
...
export class MovesetCopyMoveAttr extends OverrideMoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const thisMoveIndex = user
      .getMoveset()
      .findIndex((m) => m.moveId === move.id);
    ...
    user.summonData.moveset[thisMoveIndex] = new PokemonMove(
      copiedMove.id,
      0,
      0,
    );
...
export class SketchAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    const targetMove = target
      .getLastXMoves(-1)
      .find(
        (m) => m.move !== Moves.NONE && m.move !== Moves.STRUGGLE && !m.virtual,
      );
    if (!targetMove) {
      return false;
    }
...
export class AbilityChangeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (moveTarget.breakIllusion()) {
      globalScene.queueMessage(
        i18next.t("abilityTriggers:illusionBreak", {
...
export class AbilityCopyAttr extends MoveEffectAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) => {
      const ally = user.getAlly();
      let ret =
        target.getAbility().isCopiable && user.getAbility().isReplaceable;
...
export class SwitchAbilitiesAttr extends MoveEffectAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    globalScene.queueMessage(
      i18next.t("moveTriggers:swappedAbilitiesWithTarget", {
        pokemonName: getPokemonNameWithAffix(user),
      }),
    );
  ...
  getCondition(): MoveConditionFunc {
    return (user, target, move) =>
      [user, target].every((pkmn) => pkmn.getAbility().isSwappable);
  }
}
...
export class SuppressAbilitiesAttr extends MoveEffectAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (_user, target, _move) =>
      ...
      (target.getAbility().isSuppressable ||
        (target.hasPassive() && target.getPassiveAbility().isSuppressable));
  }
}
...
export class ShiftStatAttr extends MoveEffectAttr {
  ...
  override getUserBenefitScore(
    ...
    return user.getStat(this.statToSwitchWith, false) >
      user.getStat(this.statToSwitch, false)
      ? 10
      : 0;
  }
}
    ...
    if (super.apply(user, target, move, args)) {
      for (const s of this.stats) {
        const avg = Math.floor(
          (user.getStat(s, false) + target.getStat(s, false)) / 2,
        );
...
export class MoneyAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move): boolean {
    globalScene.currentBattle.moneyScattered +=
      globalScene.getWaveMoneyAmount(0.2);
    globalScene.queueMessage(
      i18next.t("moveTriggers:coinsScatteredEverywhere"),
    );
    return true;
  }
}
...
export class LastResortAttr extends MoveAttr {
  ...
  getCondition(): MoveConditionFunc {
    return (user: Pokemon, _target: Pokemon, move: Move) => {
      const movesInMoveset = new Set<Moves>(
        user.getMoveset().map((m) => m.moveId),
      );
...
export class VariableTargetAttr extends MoveAttr {
  ...
  constructor(
    targetChange: (user: Pokemon, target: Pokemon, move: Move) => number,
  ) {
    super();
...
export class AfterYouAttr extends MoveEffectAttr {
  ...
  override apply(
    ...
    const nextAttackPhase = globalScene.findPhase<MovePhase>(
      (phase) => phase.pokemon === target,
    );
    if (
      nextAttackPhase &&
      globalScene.tryRemovePhase((phase: MovePhase) => phase.pokemon === target)
    ) {
      globalScene.prependToPhase(
        new MovePhase(
          target,
...
export class ForceLastAttr extends MoveEffectAttr {
  ...
  override apply(
    ...
    globalScene.queueMessage(
      i18next.t("moveTriggers:forceLast", {
        targetPokemonName: getPokemonNameWithAffix(target),
      }),
    );

    const targetMovePhase = globalScene.findPhase<MovePhase>(
      (phase) => phase.pokemon === target,
    );
    if (
      targetMovePhase &&
      !targetMovePhase.isForcedLast() &&
      globalScene.tryRemovePhase((phase: MovePhase) => phase.pokemon === target)
    ) {
      ...
      const prependPhase = globalScene.findPhase(
        (phase) =>
          ...
          (phase instanceof MovePhase &&
            phaseForcedSlower(
              phase,
              target,
              !!globalScene.arena.getTag(ArenaTagType.TRICK_ROOM),
            )),
      );
      if (prependPhase) {
        globalScene.phaseQueue.splice(
          ...
          new MovePhase(
            target,
            [...targetMovePhase.targets],
            targetMovePhase.move,
            false,
            false,
            false,
            true,
...
const phaseForcedSlower = (
  ...
  if (
    phase.pokemon.getEffectiveStat(Stat.SPD) ===
    target.getEffectiveStat(Stat.SPD)
  ) {
...
const failOnBossCondition: MoveConditionFunc = (user, target, move) =>
  !target.isBossImmune();

const failIfSingleBattle: MoveConditionFunc = (user, target, move) =>
  globalScene.currentBattle.double;
...
const failIfDampCondition: MoveConditionFunc = (user, target, move) => {
  ...
  globalScene
    .getField(true)
    .map((p) => applyAbAttrs(FieldPreventExplosiveMovesAbAttr, p, cancelled));
...
const userSleptOrComatoseCondition: MoveConditionFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) =>
...
const targetSleptOrComatoseCondition: MoveConditionFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) =>
  target.status?.effect === StatusEffect.SLEEP ||
  target.hasAbility(Abilities.COMATOSE);

const failIfLastCondition: MoveConditionFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) =>
  globalScene.phaseQueue.find((phase) => phase instanceof MovePhase) !==
  undefined;

const failIfLastInPartyCondition: MoveConditionFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) => {
  const party: Pokemon[] = user.isPlayer()
    ? globalScene.getPlayerParty()
    : globalScene.getEnemyParty();
  return party.some((pokemon) => pokemon.isActive() && !pokemon.isOnField());
};

const failIfGhostTypeCondition: MoveConditionFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) => !target.isOfType(PokemonType.GHOST);

const failIfNoTargetHeldItemsCondition: MoveConditionFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) => target.getHeldItems().filter((i) => i.isTransferable)?.length > 0;

const attackedByItemMessageFunc = (
  user: Pokemon,
  target: Pokemon,
  move: Move,
) => {
...
function applyMoveAttrsInternal(
  ...
  move.attrs
    ...
    .forEach((attr) => attr.apply(user, target, move, args));
}
...
export function applyMoveAttrs(
  ...
  applyMoveAttrsInternal(
    (attr: MoveAttr) => attr instanceof attrType,
    user,
    target,
    move,
    args,
...
export class UpperHandCondition extends MoveCondition {
  constructor() {
    super((user, target, move) => {
      const targetCommand =
        globalScene.currentBattle.turnCommands[target.getBattlerIndex()];
...
export class HitsSameTypeAttr extends VariableMoveTypeMultiplierAttr {
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    const multiplier = args[0] as NumberHolder;
    if (
      !user.getTypes(true).some((type) => target.getTypes(true).includes(type))
    ) {
      multiplier.value = 0;
      return true;
    }
...
export class ResistLastMoveTypeAttr extends MoveEffectAttr {
  ...
  apply(user: Pokemon, target: Pokemon, move: Move, args: any[]): boolean {
    ...
    if (
      moveData.type === PokemonType.STELLAR ||
      moveData.type === PokemonType.UNKNOWN
    ) {
      return false;
    }
    const userTypes = user.getTypes();
    const validTypes = this.getTypeResistances(
      globalScene.gameMode,
      moveData.type,
    ).filter((t) => !userTypes.includes(t)); // valid types are ones that are not already the user's types
    ...
    globalScene.queueMessage(
      i18next.t("battle:transformedIntoType", {
        pokemonName: getPokemonNameWithAffix(user),
        type: toReadableString(PokemonType[type]),
      }),
    );
    user.updateInfo();
...
const unknownTypeCondition: MoveConditionFunc = (user, target, move) =>
...
export function getMoveTargets(
  ...
  switch (moveTarget) {
    ...
    case MoveTarget.ALL_OTHERS:
      ...
      multiple =
        moveTarget === MoveTarget.ALL_NEAR_OTHERS ||
        moveTarget === MoveTarget.ALL_OTHERS;
      break;
    ...
    case MoveTarget.ALL:
    case MoveTarget.BOTH_SIDES:
      set = (!isNullOrUndefined(ally) ? [user, ally] : [user]).concat(
        opponents,
      );
      multiple = true;
      break;
    case MoveTarget.CURSE:
      ...
      set = user.getTypes(true).includes(PokemonType.GHOST)
        ? opponents.concat(extraTargets)
        : [user];
      break;
  }
...
export function initMoves() {
  allMoves.push(
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:whippedUpAWhirlwind", {
          pokemonName: "{USER}",
        }),
      )
      .attr(HighCritAttr)
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:flewUpHigh", { pokemonName: "{USER}" }),
      )
    ...
    new StatusMove(Moves.DISABLE, PokemonType.NORMAL, 100, 20, -1, 0, 1)
      ...
      .condition((user, target, move) => {
        ...
        return (
          !isNullOrUndefined(lastRealMove) &&
          lastRealMove.move !== Moves.NONE &&
          lastRealMove.move !== Moves.STRUGGLE
        );
      })
      .ignoresSubstitute()
      .reflectable(),
    ...
    new AttackMove(
      ...
      .attr(
        CounterDamageAttr,
        (move: Move) => move.category === MoveCategory.PHYSICAL,
        2,
      )
    ...
    new StatusMove(Moves.LEECH_SEED, PokemonType.GRASS, 90, 10, -1, 0, 1)
      ...
      .condition(
        (user, target, move) =>
          !target.getTag(BattlerTagType.SEEDED) &&
          !target.isOfType(PokemonType.GRASS),
      )
      .reflectable(),
    new SelfStatusMove(Moves.GROWTH, PokemonType.NORMAL, -1, 20, -1, 0, 1).attr(
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:tookInSunlight", { pokemonName: "{USER}" }),
      )
      .chargeAttr(WeatherInstantChargeAttr, [
        WeatherType.SUNNY,
    ...
    new SelfStatusMove(Moves.RECOVER, PokemonType.NORMAL, -1, 5, -1, 0, 1)
      .attr(HealAttr, 0.5)
      .triageMove(),
    new SelfStatusMove(Moves.HARDEN, PokemonType.NORMAL, -1, 30, -1, 0, 1).attr(
      StatStageChangeAttr,
      [Stat.DEF],
      1,
      true,
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:loweredItsHead", { pokemonName: "{USER}" }),
      )
    ...
    new StatusMove(Moves.TRANSFORM, PokemonType.NORMAL, -1, 10, -1, 0, 1)
      .attr(TransformAttr)
      .condition(
        (user, target, move) => !target.getTag(BattlerTagType.SUBSTITUTE),
      )
      .condition(
        (user, target, move) =>
          !target.summonData.illusion && !user.summonData.illusion,
      )
    ...
    new StatusMove(Moves.SPIDER_WEB, PokemonType.BUG, -1, 10, -1, 0, 2)
      ...
      .reflectable(),
    new StatusMove(Moves.MIND_READER, PokemonType.NORMAL, -1, 5, -1, 0, 2).attr(
      IgnoreAccuracyAttr,
    ),
    ...
    new AttackMove(
      ...
      .attr(MultiHitAttr)
      .makesContact(false),
    new StatusMove(Moves.LOCK_ON, PokemonType.NORMAL, -1, 5, -1, 0, 2).attr(
      IgnoreAccuracyAttr,
    ),
    new AttackMove(
      Moves.OUTRAGE,
    ...
    new StatusMove(Moves.HEAL_BELL, PokemonType.NORMAL, -1, 5, -1, 0, 2)
      .attr(
        PartyStatusCureAttr,
        i18next.t("moveTriggers:bellChimed"),
    ...
    new AttackMove(
      ...
      .attr(
        CounterDamageAttr,
        (move: Move) => move.category === MoveCategory.SPECIAL,
        2,
      )
      .target(MoveTarget.ATTACKER),
    ...
    new SelfStatusMove(Moves.STOCKPILE, PokemonType.NORMAL, -1, 20, -1, 0, 3)
      .condition(
        (user) => (user.getTag(StockpilingTag)?.stockpiledCount ?? 0) < 3,
      )
    ...
    new AttackMove(
      ...
      .attr(MessageHeaderAttr, (user, move) =>
        i18next.t("moveTriggers:isTighteningFocus", {
          pokemonName: getPokemonNameWithAffix(user),
        }),
      )
      .attr(
        PreUseInterruptAttr,
        (user, target, move) =>
          i18next.t("moveTriggers:lostFocus", {
            pokemonName: getPokemonNameWithAffix(user),
          }),
        (user) => !!user.turnData.attacksReceived.find((r) => r.damage),
      )
      .punchingMove(),
    new AttackMove(
      Moves.SMELLING_SALTS,
      PokemonType.NORMAL,
      MoveCategory.PHYSICAL,
      70,
      100,
    ...
    new SelfStatusMove(Moves.ASSIST, PokemonType.NORMAL, -1, 20, -1, 0, 3).attr(
    ...
    new StatusMove(Moves.YAWN, PokemonType.NORMAL, -1, 10, -1, 0, 3)
      ...
      .condition(
        (user, target, move) => !target.status && !target.isSafeguarded(user),
      )
      .reflectable(),
    new AttackMove(
      Moves.KNOCK_OFF,
    ...
    new SelfStatusMove(Moves.GRUDGE, PokemonType.GHOST, -1, 5, -1, 0, 3).attr(
      AddBattlerTagAttr,
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:hidUnderwater", { pokemonName: "{USER}" }),
      )
    ...
    new SelfStatusMove(Moves.TAIL_GLOW, PokemonType.BUG, -1, 20, -1, 0, 3).attr(
      StatStageChangeAttr,
      [Stat.SPATK],
      3,
      true,
    ...
    new StatusMove(Moves.AROMATHERAPY, PokemonType.GRASS, -1, 5, -1, 0, 3)
      .attr(
        PartyStatusCureAttr,
        i18next.t("moveTriggers:soothingAromaWaftedThroughArea"),
    ...
    new AttackMove(
      ...
      .attr(
        ...
        i18next.t("moveTriggers:choseDoomDesireAsDestiny", {
          pokemonName: "{USER}",
        }),
      ),
    new AttackMove(
    ...
    new SelfStatusMove(
      ...
      .attr(
        ...
        false,
        "moveTriggers:sacrificialFullRestore",
      )
      .triageMove()
      .condition(failIfLastInPartyCondition),
    new AttackMove(
    ...
    new AttackMove(
      ...
      target.turnData.damageTaken > 0 ? 2 : 1,
    ),
    ...
    new StatusMove(Moves.PSYCHO_SHIFT, PokemonType.PSYCHIC, 100, 10, -1, 0, 4)
      ...
      .condition((user, target, move) => {
        let statusToApply = user.hasAbility(Abilities.COMATOSE)
          ? StatusEffect.SLEEP
          : undefined;
        if (
          user.status?.effect &&
          isNonVolatileStatusEffect(user.status.effect)
        ) {
          statusToApply = user.status.effect;
        }
        return (
          !!statusToApply &&
          target.canSetStatus(statusToApply, false, false, user)
        );
      }),
    new AttackMove(
      Moves.TRUMP_CARD,
    ...
    new StatusMove(Moves.DEFOG, PokemonType.FLYING, -1, 15, -1, 0, 4)
      ...
      .attr(
        RemoveArenaTagsAttr,
        [ArenaTagType.MIST, ArenaTagType.SAFEGUARD],
        false,
      )
      .reflectable(),
    ...
    new AttackMove(
      ...
      target.status &&
      (target.status.effect === StatusEffect.POISON ||
        target.status.effect === StatusEffect.TOXIC)
        ? 2
        : 1,
    ),
    ...
    new StatusMove(Moves.TELEKINESIS, PokemonType.PSYCHIC, -1, 15, -1, 0, 5)
      ...
      .condition(
        (_user, target, _move) =>
          !(
            target.species.speciesId === Species.GENGAR &&
            target.getFormKey() === "mega"
          ),
      )
      .condition(
        (_user, target, _move) =>
    ...
    new SelfStatusMove(Moves.COIL, PokemonType.POISON, -1, 20, -1, 0, 5).attr(
      StatStageChangeAttr,
    ...
    new AttackMove(
      ...
      target.status || target.hasAbility(Abilities.COMATOSE) ? 2 : 1,
    ),
    new ChargingAttackMove(
      Moves.SKY_DROP,
      PokemonType.FLYING,
      ...
      .chargeText(
        i18next.t("moveTriggers:tookTargetIntoSky", {
          pokemonName: "{USER}",
          targetName: "{TARGET}",
        }),
      )
    ...
    new AttackMove(
      ...
      const lastPlayerFaint =
        globalScene.currentBattle.playerFaintsHistory[
          globalScene.currentBattle.playerFaintsHistory.length - 1
        ];
      const lastEnemyFaint =
        globalScene.currentBattle.enemyFaintsHistory[
          globalScene.currentBattle.enemyFaintsHistory.length - 1
        ];
      return (lastPlayerFaint !== undefined &&
        turn - lastPlayerFaint.turn === 1 &&
        user.isPlayer()) ||
        (lastEnemyFaint !== undefined &&
          turn - lastEnemyFaint.turn === 1 &&
          !user.isPlayer())
        ? 2
        : 1;
    }),
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:becameCloakedInFreezingLight", {
          pokemonName: "{USER}",
        }),
      )
      .attr(StatusEffectAttr, StatusEffect.PARALYSIS)
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:becameCloakedInFreezingAir", {
          pokemonName: "{USER}",
        }),
      )
      .attr(StatusEffectAttr, StatusEffect.BURN),
    ...
    new StatusMove(Moves.ROTOTILLER, PokemonType.GROUND, -1, 10, -1, 0, 6)
      ...
      .condition((user, target, move) => {
        ...
        return [
          ...globalScene.getEnemyParty(),
          ...globalScene.getPlayerParty(),
        ].some((poke) => poke.isOfType(PokemonType.GRASS) && poke.isGrounded());
      })
      .attr(StatStageChangeAttr, [Stat.ATK, Stat.SPATK], 1, false, {
        condition: (user, target, move) =>
          target.isOfType(PokemonType.GRASS) && target.isGrounded(),
      }),
    ...
    new StatusMove(Moves.FLOWER_SHIELD, PokemonType.FAIRY, -1, 10, -1, 0, 6)
      ...
      .attr(StatStageChangeAttr, [Stat.DEF], 1, false, {
        condition: (user, target, move) =>
          target.getTypes().includes(PokemonType.GRASS) &&
          !target.getTag(SemiInvulnerableTag),
      }),
    ...
    new ChargingSelfStatusMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:isChargingPower", { pokemonName: "{USER}" }),
      )
    ...
    new StatusMove(Moves.MAGNETIC_FLUX, PokemonType.ELECTRIC, -1, 20, -1, 0, 6)
      .attr(StatStageChangeAttr, [Stat.DEF, Stat.SPDEF], 1, false, {
        condition: (user, target, move) =>
          !![Abilities.PLUS, Abilities.MINUS].find((a) =>
            target.hasAbility(a, false),
          ),
      })
      .ignoresSubstitute()
      .target(MoveTarget.USER_AND_ALLIES)
    ...
    new AttackMove(
      ...
      .attr(RemoveTypeAttr, PokemonType.FIRE, (user) => {
        globalScene.queueMessage(
          i18next.t("moveTriggers:burnedItselfOut", {
            pokemonName: getPokemonNameWithAffix(user),
          }),
        );
      }),
    ...
    new AttackMove(
      ...
      .condition(
        (user, target, move) => user.getTag(ShellTrapTag)?.activated === true,
      ),
    new AttackMove(
      Moves.FLEUR_CANNON,
      PokemonType.FAIRY,
    ...
    new AttackMove(
      ...
      user.getLastXMoves(2)[1]?.result === MoveResult.MISS ||
      user.getLastXMoves(2)[1]?.result === MoveResult.FAIL
        ? 2
        : 1,
    ),
    new AttackMove(
        ...
        return (
          ...
          Math.min(
            ...
            (target.level - globalScene.getMaxExpLevel()) /
              (globalScene.getMaxExpLevel() *
                dynamaxCannonPercentMarginBeforeFullDamage),
          )
        );
      } else {
        return 1;
    ...
    new SelfStatusMove(Moves.STUFF_CHEEKS, PokemonType.NORMAL, -1, 10, -1, 0, 8)
      ...
      .attr(StatStageChangeAttr, [Stat.DEF], 2, true)
      .condition((user) => {
        const userBerries = globalScene.findModifiers(
    ...
    new SelfStatusMove(Moves.NO_RETREAT, PokemonType.FIGHTING, -1, 5, -1, 0, 8)
      ...
      .condition(
        (user, target, move) =>
          user.getTag(TrappedTag)?.sourceMove !== Moves.NO_RETREAT,
      ), // fails if the user is currently trapped by No Retreat
    ...
    new AttackMove(
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        globalScene.arena.getTerrainType() === TerrainType.PSYCHIC &&
        user.isGrounded()
          ? 1.5
          : 1,
      )
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:isOverflowingWithSpacePower", {
          pokemonName: "{USER}",
        }),
      )
    ...
    new AttackMove(
      ...
      .target(MoveTarget.ALL_NEAR_OTHERS)
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        globalScene.arena.getTerrainType() === TerrainType.MISTY &&
        user.isGrounded()
          ? 1.5
          : 1,
      )
    ...
    new AttackMove(
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        globalScene.arena.getTerrainType() !== TerrainType.NONE &&
        user.isGrounded()
          ? 2
          : 1,
      )
    ...
    new AttackMove(
      ...
      user.turnData.statStagesDecreased ? 2 : 1,
    ),
    new AttackMove(
      Moves.POLTERGEIST,
      PokemonType.GHOST,
    ...
    new SelfStatusMove(Moves.SHELTER, PokemonType.STEEL, -1, 10, -1, 0, 8).attr(
      StatStageChangeAttr,
      [Stat.DEF],
      2,
      true,
    ...
    new AttackMove(
      ...
      .attr(StatStageChangeAttr, [Stat.ATK, Stat.SPATK], -1, true, {
        condition: (user, target, move) =>
          user.isTerastallized && user.isOfType(PokemonType.STELLAR),
      }),
    ...
    new AttackMove(
      ...
      .condition((user, target, move) => {
        return !(
          ...
          globalScene.arena.getTag(ArenaTagType.MAT_BLOCK)?.tagType ===
            "MAT_BLOCK"
        );
      }),
    ...
    new AttackMove(
      ...
      .attr(RemoveArenaTrapAttr)
      .target(MoveTarget.ALL_NEAR_ENEMIES),
    new StatusMove(Moves.DOODLE, PokemonType.NORMAL, 100, 10, -1, 0, 9).attr(
      AbilityCopyAttr,
      true,
    ),
    new SelfStatusMove(
    ...
    new AttackMove(
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) => {
        ...
        return [WeatherType.SUNNY, WeatherType.HARSH_SUN].includes(
          weather.weatherType,
        ) && !weather.isEffectSuppressed()
          ? 1.5
          : 1;
    ...
    new AttackMove(
      ...
      .attr(MovePowerMultiplierAttr, (user, target, move) =>
        target.getAttackTypeEffectiveness(move.type, user) >= 2
          ? 5461 / 4096
          : 1,
      )
      .makesContact(),
    ...
    new SelfStatusMove(
      ...
      .attr(PreMoveMessageAttr, (user, move) =>
        i18next.t("moveTriggers:chillyReception", {
          pokemonName: getPokemonNameWithAffix(user),
        }),
      )
    ...
    new AttackMove(
      ...
      .attr(RemoveTypeAttr, PokemonType.ELECTRIC, (user) => {
        globalScene.queueMessage(
          i18next.t("moveTriggers:usedUpAllElectricity", {
            pokemonName: getPokemonNameWithAffix(user),
          }),
        );
      }),
    new AttackMove(
      ...
      .condition((user, target, move) => {
        ...
        return (
          !turnMove.length ||
          turnMove[0].move !== move.id ||
          turnMove[0].result !== MoveResult.SUCCESS
        );
    ...
    new ChargingAttackMove(
      ...
      .chargeText(
        i18next.t("moveTriggers:absorbedElectricity", {
          pokemonName: "{USER}",
        }),
      )
      .chargeAttr(StatStageChangeAttr, [Stat.SPATK], 1, true)
    ...
    new AttackMove(
      ...
      .attr(VariableTargetAttr, (user, target, move) =>
        user.hasSpecies(Species.TERAPAGOS) &&
        (user.isTerastallized ||
          globalScene.currentBattle.preTurnCommands[user.getFieldIndex()]
  ...
  allMoves.map((m) => {
    if (
      m.getAttrs(StatStageChangeAttr).some((a) => a.selfTarget && a.stages < 0)
    ) {
