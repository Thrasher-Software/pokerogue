...
function parseEggMoves(content: string): void {
  ...
  for (const line of lines) {
    ...
    const moveNames = allMoves.map((m) =>
      m.name.replace(/ \([A-Z]\)$/, "").toLowerCase(),
    );
    const enumSpeciesName = cols[0].toUpperCase().replace(/[ -]/g, "_");
    const species =
      speciesValues[speciesNames.findIndex((s) => s === enumSpeciesName)];
    ...
    for (let m = 0; m < 4; m++) {
      ...
      const moveIndex =
        moveName !== "N/A"
          ? moveNames.findIndex((mn) => mn === moveName.toLowerCase())
          : -1;
